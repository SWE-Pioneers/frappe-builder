	...
	<div class="group flex w-full flex-col gap-2">
		<div
			...
			<div
				...
				<Button size="sm" variant="solid" @click.stop="$emit('select', page)">{{ __("Use template") }}</Button>
				<Button size="sm" variant="subtle" @click.stop="openPreview">{{ __("Preview") }}</Button>
			</div>
		</div>
		<div class="flex items-center justify-between gap-2 px-[2px]">
			...
			<Button
				v-if="isDeveloperMode && !page.live_url"
				size="sm"
				variant="ghost"
				icon="lucide-pencil"
